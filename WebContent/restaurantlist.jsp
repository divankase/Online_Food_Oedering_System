...
<%@include file="header.jsp" %>
		<div class="container" style="margin-left: 20%; padding-top:5%">
			<h3 class="text-center">List of Restaurant Details</h3>
			<hr>
			<div class="container text-left">

				<a href="<%=request.getContextPath()%>/newrestaurantdetails" class="btn btn-secondary">Add Restaurant Details</a>
			</div>
			
				<div class="container text-right">

				<a href="<%=request.getContextPath()%>/restaurantdetailspdf" class="btn btn-secondary">Generate Report</a>
			</div>
			<br>
			<table id="dtBasicExample" class="table table-striped table-bordered table-sm">
				...
				<tbody>
					...
					<c:forEach var="user" items="${listrestaurantdetails}">
						<tr>
						<td><c:out value="${user.restaurantname}" /></td>	
							...
							<td><a href="editrestaurantdetails?id=<c:out value='${user.id}' />">Edit</a>
								&nbsp;&nbsp;&nbsp;&nbsp; <a
								href="deleterestaurantdetails?id=<c:out value='${user.id}' />">Delete</a></td>
						</tr>
					</c:forEach>
					<!-- } -->
				</tbody>
